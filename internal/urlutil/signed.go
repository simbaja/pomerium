...
func (su *SignedURL) Validate() error {
	...
	if err != nil {
		return fmt.Errorf("issued: %v: found: %v, params: %v", err, params.Get(QueryHmacIssued), params)
	}

	expiry, err := newNumericDateFromString(params.Get(QueryHmacExpiry))
	if err != nil {
		return fmt.Errorf("expiry: %v: found: %v, params: %v", err, params.Get(QueryHmacExpiry), params)
	}
